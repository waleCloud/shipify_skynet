<?php
	...
	/*function is_valid_request($query_params, $shared_secret)
	{
		...
		$signature = $query_params['timestamp'];
	...
	function authorization_url($shop, $api_key, $scopes=array(), $redirect_uri='http://latsil.com/shipify_skynet/oauth.php')
	{
		$scopes = empty($scopes) ? '' : '&scope='.implode(',', $scopes);
		$redirect_uri = empty($redirect_uri) ? '' : '&redirect_uri='.urlencode($redirect_uri);
		return "https://$shop/admin/oauth/authorize?client_id=$api_key$scopes&redirect_uri=$redirect_uri";
	}
